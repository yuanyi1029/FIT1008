...
class MyWindow(arcade.Window):
    ...
    def init_graph(self):
        ...
        self.graph.add(gui.UIAnchorWidget(
            anchor_x="center",
            anchor_y="center",
            child=gui.UISpace(width=self.GRAPH_WIDTH + self.LABEL_WIDTH, height=self.GRAPH_HEIGHT,
    ...
    def draw_graph_elems(self):
        ...
        for color, start_index, name, positions in self.graph_data:
            ...
            arcade.draw_text(name, points[-1][0] + 0.5 * self.GRAPH_WIDTH / total_x_points + 5, points[-1][1],
                             color=(0, 0, 0), anchor_y="center")
    ...
    def on_draw(self) -> None:
        ...
        if self.draw_box is not None and not (self.showing_graph or self.is_editing or self.is_saving):
            arcade.draw_rectangle_filled(self.draw_box.x + self.draw_box.w / 2, self.draw_box.y + self.draw_box.h / 2,
                                         self.draw_box.w, self.draw_box.h, (0, 255, 0, 100))
        ...
        for i, ((path, button_type, tracker), button) in enumerate(zip(self.ACTIONS, self.action_buttons)):
            if button_type == "toggle" or button_type == "mode":
                if getattr(self, tracker):
                    arcade.draw_rectangle_filled(button.center_x, button.center_y, self.LAYER_BUTTON_SIZE,
                                                 self.LAYER_BUTTON_SIZE, (0, 255, 0, 100))
        if self.is_editing:
            self.manager.draw()
