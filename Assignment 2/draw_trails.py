...
class TrailDraw:
    ...
    def required_height(self, cur_trail: TrailBox | None = None) -> int:
        ...
        if cur_trail is None:
            ...
            return max(
                self.required_height(cur_trail.path_top) + self.BRANCH_SEPARATION + self.required_height(
                    cur_trail.path_bottom),
    ...
    def draw_in_box(self, height, width, minx, miny, cur_trail: TrailBox | None = None) -> None:
        ...
        if cur_trail is None:
            ...
            ref_trail.trail_box = Box(minx, miny + height / 2 - self.LINE_VERTICAL_BOX, width,
                                      2 * self.LINE_VERTICAL_BOX)
        elif isinstance(cur_trail, TrailSeries):
            ...
            mid = miny + height / 2
            self.draw_mountain(av(start_mountain_x, end_mountain_x), mid,
                               (end_mountain_x - start_mountain_x) / self.MIN_MOUNTAIN_WIDTH, cur_trail.mountain)
            self.draw_line(start_mountain_trail_x, mid, start_mountain_x, mid)
            self.draw_line(end_mountain_x, mid, end_mountain_trail_x, mid)
            mountain_actual_height = self.MOUNTAIN_HEIGHT * (
                        end_mountain_x - start_mountain_x) / self.MIN_MOUNTAIN_WIDTH
            cur_trail.before_box = Box(start_mountain_trail_x, mid - mountain_actual_height / 2,
                                       start_mountain_x - start_mountain_trail_x, mountain_actual_height)
            cur_trail.mountain_box = Box(start_mountain_x, mid - mountain_actual_height / 2,
                                         end_mountain_x - start_mountain_x, mountain_actual_height)
            cur_trail.after_box = Box(end_mountain_x, mid - mountain_actual_height / 2,
                                      end_mountain_trail_x - end_mountain_x, mountain_actual_height)
            # Draw rest
            ...
            # Draw branches
            self.draw_branch(minx, mid, minx + self.BRANCH_WIDTH,
                             miny + bot_section + self.BRANCH_SEPARATION + top_section / 2, miny + bot_section / 2)
            self.draw_branch(minx + width - b3_dist, mid, minx + width - self.BRANCH_WIDTH - b3_dist,
                             miny + bot_section + self.BRANCH_SEPARATION + top_section / 2, miny + bot_section / 2)
            cur_trail.branch_start_box = Box(minx, mid - self.BRANCH_SEPARATION / 2 - top_section / 2,
                                             self.BRANCH_WIDTH,
                                             bot_section / 2 + top_section / 2 + self.BRANCH_SEPARATION)
            cur_trail.branch_end_box = Box(minx + width - b3_dist - self.BRANCH_WIDTH,
                                           mid - self.BRANCH_SEPARATION / 2 - top_section / 2, self.BRANCH_WIDTH,
                                           bot_section / 2 + top_section / 2 + self.BRANCH_SEPARATION)
            # Draw top & bottom
            self.draw_in_box(top_section, branch_dist, minx + self.BRANCH_WIDTH,
                             miny + bot_section + self.BRANCH_SEPARATION, cur_trail.path_top)
    ...
    def box_and_action(self, mouse_pos: tuple[float, float], mode=DrawMode, cur_trail: Trail | None = None,
                       parent_sets: tuple[Trail, str] | None = None) -> tuple[
        Box | None, function | None, Trail | None]:
        if cur_trail is None:
            ref_trail = self.trail
            cur_trail = self.trail.store
        ...
        if cur_trail is None:
            if mode in [DrawMode.ADD_MOUNTAIN, DrawMode.ADD_BRANCH]:
                return ref_trail.trail_box, set_parent(parent_sets,
                                                       ref_trail.add_mountain_before if mode == DrawMode.ADD_MOUNTAIN else ref_trail.add_empty_branch_before), cur_trail
        elif isinstance(cur_trail, TrailSeries):
            if mouse_pos in cur_trail.before_box and mode in [DrawMode.ADD_MOUNTAIN, DrawMode.ADD_BRANCH]:
                return cur_trail.before_box, set_m(ref_trail,
                                                   cur_trail.add_mountain_before if mode == DrawMode.ADD_MOUNTAIN else cur_trail.add_empty_branch_before), cur_trail
            if mouse_pos in cur_trail.mountain_box and mode in [DrawMode.REMOVE, DrawMode.EDIT]:
                return cur_trail.mountain_box, (set_m(ref_trail,
                                                      cur_trail.remove_mountain) if mode == DrawMode.REMOVE else lambda: cur_trail.mountain), cur_trail
            if mouse_pos in cur_trail.after_box and mode in [DrawMode.ADD_MOUNTAIN, DrawMode.ADD_BRANCH]:
                return cur_trail.after_box, set_m(ref_trail,
                                                  cur_trail.add_mountain_after if mode == DrawMode.ADD_MOUNTAIN else cur_trail.add_empty_branch_after), cur_trail
